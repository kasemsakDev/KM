...
while($row = mysqli_fetch_assoc($sql_resultyear))
    ...
    Where i.IsActive = 1 AND a.IsActive = 1 AND i.AgencyID = $agencyId AND p.IsActive = 1 AND YEAR(i.CreateOn) = '$Whereyear'";
    ...
    //get purpose
    $sql_listpurpose = "SELECT 	PurposeID,Name,AgencyID FROM km_purpose WHERE IsActive  = 1 AND AgencyID = $agencyId AND YEAR(km_purpose.CreateOn) = '$Whereyear'";
    ...
    if(isset($_GET['id']))
    {
        ...
        INNER JOIN  km_agency a on i.AgencyID = a.AgencyID
        INNER JOIN  km_purpose p on p.PurposeID = i.PurposeID 
        Where i.IsActive = 1 AND a.IsActive = 1 AND i.AgencyID = $_getId AND p.IsActive = 1 AND YEAR(i.CreateOn) = '$Whereyear'";
        $sql_resultindicator =  mysqli_query($link,$sql_getindicator); 
        while($row = mysqli_fetch_assoc($sql_resultindicator))
        {
            $indicator[] = $row;
...
<script>
        ...
        var value = e.value;
        window.location.href = 'indicator.php?id='+value+'&year='+year;
        }
